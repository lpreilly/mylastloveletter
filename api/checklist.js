...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    url = `${sb_url}/rest/v1/checklist_items?post_id=eq.${post_id}&select=*&order=sort_order.asc`;
    options = { headers };
  } else if (req.method === 'POST') {
    url = `${sb_url}/rest/v1/checklist_items`;
    options = { method: 'POST', headers: { ...headers, 'Prefer': 'return=representation' }, body: JSON.stringify(req.body) };
  } else if (req.method === 'PUT') {
    url = `${sb_url}/rest/v1/checklist_items?id=eq.${id}`;
